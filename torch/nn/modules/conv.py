    ...
    def __init__(self,
        ...
        if transposed:
            self.weight = Parameter(torch.empty(
                (in_channels, out_channels // groups, *kernel_size), **factory_kwargs, weightEmpty=True))
        else:
            self.weight = Parameter(torch.empty(
                (out_channels, in_channels // groups, *kernel_size), **factory_kwargs, weightEmpty=True))
        if bias:
            self.bias = Parameter(torch.empty(out_channels, **factory_kwargs, weightEmpty=True))
        else:
