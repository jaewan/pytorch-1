...
static hipStream_t low_priority_streams[C10_COMPILE_TIME_MAX_GPUS]
                                        [kStreamsPerPool];
static hipStream_t high_priority_streams[C10_COMPILE_TIME_MAX_GPUS]
                                         [kStreamsPerPool];
...
HIPStream getStreamFromPool(
  ...
  if (isHighPriority) {
    ...
    return HIPStreamForId(device_index, makeStreamId(StreamIdType::HIGH, idx));
  }

  const auto idx = get_idx(low_priority_counters[device_index]);
  return HIPStreamForId(device_index, makeStreamId(StreamIdType::LOW, idx));
}
